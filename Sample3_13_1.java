public class Sample3_13_1 {                                      // 宣告公開類別 Sample3_13_1（檔名需與類別同名）
    public static void main(String[] args) {                      // 主程式進入點（JVM 從這裡開始執行）
        Car car1 = new Car();                                     // 建立一台預設車（無參數建構子：車號 0、油量 0.0）
        car1.show();                                              // 顯示 car1 目前狀態（車號與汽油量）

        Car car2 = new Car(1234, 25.0);                           // 建立一台指定車號與油量的車（參數建構子）
...
final class Car {                                                // 宣告 Car 類別並標示為 final（此範例不需被繼承，保持簡潔）
    ...
    public Car(int n, double g) {                                // 參數建構子：建立時直接指定車號與汽油量
        ...
        System.out.printf("生產了車號為 %d，汽油量為 %.1f 的車子%n", n, g); // 與題目一致：建立指定值車時輸出訊息
    }                                                            // 參數建構子結束
    ...
    private static void validateGas(double g) {                  // 私有工具方法：檢核汽油量
        if (g < MIN_GAS || g > MAX_GAS) {                        // 若油量不在 [0, 100] 範圍內
            throw new IllegalArgumentException(                  // 拋出參數例外並說明合法區間
                String.format("汽油量需介於 %.1f 與 %.1f 公升之間", MIN_GAS, MAX_GAS)); // 產生友善錯誤訊息
        }                                                        // 檢核結束
    }                                                            // validateGas 方法結束

    public void show() {                                         // 成員方法：顯示目前車輛狀態
        System.out.printf("車號是 %d%n", this.num);              // 使用格式化輸出顯示車號（%n 跨平台換行）
